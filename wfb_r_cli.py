    ...
    def print_status(d_pkt, d_fec_r, d_pkts, d_pkt_lost, d_d_err, d_fec_r_s, d_pkt_lost_s, d_d_err_s, d_ant_data) -> Table:
        ...
        for ant in ant_data:
            ...
            if d_rssi_avg > -50:
                ...
                prgs_color='red'
            progress = Progress(TextColumn("[progress.description]{task.description}"), BarColumn(complete_style=prgs_color), TaskProgressColumn(text_format='[progress.percentage]{task.percentage:>3.0f}%',style='green'))
            task = progress.add_task(f"[grey58]{d_ant_num:3}", total=106)
